...
module.exports.printInventory = function (inputs){
    ...
    cart = this.getCommodityAndCount(cart, inputs);  // { barcode: 'ITEM000001', count: 5 },{...}]
    cart = this.getAllGoodsInfor(cart);
    let promotionsGoodsCart = this.getPromotionsInfor(cart);
    let totalInfor = this.getTotalInfor(cart, promotionsGoodsCart);
    this.printf(totalInfor, cart, promotionsGoodsCart);

};


module.exports.formatInput = function formatInput(inputs){
...
module.exports.getCommodityAndCount = function getCommodityAndCount(cart, inputs){
    /* 统计购买商品的个数，返回类型格式时
    [{"barcode": 商品编号1, "count": 数量},
    {"barcode": 商品编号2, "count": 数量}]
    */

    function getExistsBarcode(barcode){
...
module.exports.getAllGoodsInfor = function getAllGoodsInfor(cart){
    ...
    for(let element of cart){
        ...
        for(let item of allItems){
            if(element["barcode"] == item["barcode"]){
                ...
                goods["subtotal"] = element["count"] * item["price"];
            }
        }
        newCart.push(goods);
    }
...
module.exports.getPromotionsInfor = function getPromotionsInfor(cart){
...
module.exports.getTotalInfor = function getTotalInfor(cart, promotionsGoodsCart){
    let totalInfor = {};
    let total = 0.0;
    let promotions = 0;
    for(let element of cart){
...
module.exports.printf = function printf(totalInfor, cart, promotionsGoodsCart){
